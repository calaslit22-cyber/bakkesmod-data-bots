...
std::shared_ptr<ImageWrapper> RLProfilePictures::GetImageForPlayer(const RLProfilePictures::Pri pri) {
    // Bots use local profile pictures instead of the online profile-picture service.
    if (pri.isBot) {
        const std::string cacheKey = "BOT:" + pri.name;

        {
            std::lock_guard<std::mutex> lock(imageCacheMutex);
            auto it = botImageCache.find(cacheKey);
            if (it != botImageCache.end()) {
                return it->second;
            }
        }

        // Use dot.png for a bot whose actual in-game name is ".".
        const std::string fileName = (pri.name == ".") ? "dot.png" : pri.name + ".png";
        const std::filesystem::path botImagePath =
            gameWrapper->GetDataFolder() / "bots" / fileName;

        if (!std::filesystem::exists(botImagePath)) {
            LOG("[RLProfilePictures] No local bot profile picture found for '{}' at {}",
                pri.name, botImagePath.string());

            std::lock_guard<std::mutex> lock(imageCacheMutex);
            botImageCache[cacheKey] = nullptr;
            return nullptr;
        }

        // Load the image for Canvas immediately so it is ready when the scoreboard is drawn.
        auto image = std::make_shared<ImageWrapper>(botImagePath, true, false);

        {
            std::lock_guard<std::mutex> lock(imageCacheMutex);
            botImageCache[cacheKey] = image;
        }

        LOG("[RLProfilePictures] Loaded local bot profile picture for '{}' from {}",
            pri.name, botImagePath.string());

        return image;
    }

    // Human players keep the original online profile-picture behavior.
...
void RLProfilePictures::RenderPlatformLogos(CanvasWrapper canvas) {
    ...
    // RLProfilePictures originally only rendered during online matches.
    // Bots are normally encountered in offline/private matches, so allow
    // scoreboard rendering there too. Only perform online-server checks
    // when an online game actually exists.
