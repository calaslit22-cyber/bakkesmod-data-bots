...
private:
    ...
    // Local profile pictures for bots. Files are read from:
    // %APPDATA%\\bakkesmod\\bakkesmod\\data\\bots
    // The bot's in-game name is used as the PNG filename.
    // The special bot name "." maps to "dot.png".
    std::unordered_map<std::string, std::shared_ptr<ImageWrapper>> botImageCache;
